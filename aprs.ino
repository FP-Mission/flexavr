...
// APRS_Serial.print(F("AT+DMOSETGROUP=1,144.8,144.8,0,0,0\r\n"));
...
void CheckAPRS(void)
{
  if ((millis() >= NextAPRS) && (GPS.Satellites >= 4) && Settings.APRS_Callsign[0] && (Settings.APRS_SSID > 0) && (_txlen == 0))
  {   
    unsigned long Seconds;
    
    Serial.println(F("Setting APRS parameters"));
...
void tx_aprs(void)
{
  ...
  if (aprs_mode == 0)
  {
    ...
    ax25_frame(
      ...
      aprs_alt, stlm, F("pi-in-the-sky.com"));  // comment,APRS_CALLSIGN, ++APRSSentenceCounter);
